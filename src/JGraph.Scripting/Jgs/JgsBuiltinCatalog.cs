...
public static class JgsBuiltinCatalog
{
    ...
    private static IReadOnlyDictionary<string, JgsBuiltinInfo> Build()
    {
        ...
        Add("discretize", "Which bin each value falls in: [bin, edges] = discretize(x, edges | n, 'IncludedEdge', 'left' | 'right').", P("x"), P("bins"), Opt("values"), Opt("option"), Opt("value"));
        ...
        Add("rescale", "The whole array stretched onto an interval: rescale(x), rescale(x, l, u), 'InputMin' and 'InputMax' to fix the source range.", P("x"), Opt("low"), Opt("high"), Opt("option"), Opt("value"));
        Add("fillmissing", "Gaps filled: [f, tf] = fillmissing(x, 'linear' | 'previous' | 'next' | 'nearest' | 'constant' | 'movmean' | 'movmedian', setting).", P("x"), Opt("method"), Opt("setting"), Opt("option"), Opt("value"));
        Add("rmmissing", "Missing entries dropped, or whole rows of a matrix: [r, tf] = rmmissing(x, dim, 'MinNumMissing', k).", P("x"), Opt("dim"), Opt("option"), Opt("value"));
        Add("islocalmax", "Local maxima and their prominence: [tf, p] = islocalmax(x, 'MinProminence', p, 'MinSeparation', s, 'FlatSelection', how, 'MaxNumExtrema', n).", P("x"), Opt("dim"), Opt("option"), Opt("value"));
        Add("islocalmin", "Local minima and their prominence, with the same options as islocalmax.", P("x"), Opt("dim"), Opt("option"), Opt("value"));
        Add("smoothdata", "Smoothed data and the window it used: [b, k] = smoothdata(x, dim, 'movmean' | 'movmedian' | 'gaussian' | 'lowess' | 'loess' | 'rlowess' | 'rloess' | 'sgolay', k).", P("x"), Opt("dim"), Opt("method"), Opt("window"), Opt("option"), Opt("value"));
        ...
        Add("mle", "Maximum likelihood estimates: mle(data), mle(data, 'distribution', name), or mle(data, 'pdf', @f, 'start', p0), with 'Alpha', 'Censoring' and 'Frequency'.", P("data"), Opt("option"), Opt("value"));
        ...
        Add("writematrix", "Writes a matrix to a delimited text file: writematrix(A, 'x.csv', 'Delimiter', 'tab', 'WriteMode', 'append').", P("A"), P("path"), Opt("option"), Opt("value"));
        Add("writecell", "Writes a cell array to a delimited text file, each element as its own field.", P("C"), P("path"), Opt("option"), Opt("value"));
        Add("writetable", "Writes a table to a delimited text file; 'WriteVariableNames', false leaves the header off.", P("T"), P("path"), Opt("option"), Opt("value"));
        ...
        Add("readmatrix", "Reads a delimited text file as a numeric matrix; anything unparseable reads as NaN.", P("path"), Opt("option"), Opt("value"));
        Add("readcell", "Reads a delimited text file as a cell array, numbers as numbers and everything else as text.", P("path"), Opt("option"), Opt("value"));
        ...
        Add("polarhistogram", "Histogram of angles in radians: polarhistogram(theta, nbins | edges, 'Normalization', how, 'DisplayStyle', 'bar' | 'stairs').", P("theta"), Opt("bins"), Opt("option"), Opt("value"));
        ...
        Add("boxchart", "A box and whiskers per group: boxchart(ydata), boxchart(xgroupdata, ydata), 'GroupByColor', c.", P("ydata"), Opt("ydata"), Opt("option"), Opt("value"));
        ...
        Add("bubblelegend", "Legends the bubble sizes: bubblelegend, bubblelegend(title), 'Style', 'telescopic'.", Opt("title"), Opt("option"), Opt("value"));
