...
internal static partial class JgsBuiltins
{
    ...
    /// <summary>
    /// MATLAB's words when <c>^</c> is handed an operand with more than two dimensions. It is a
    /// separate sentence from <see cref="MPowerShapeRefusal"/> there, and measured to be: an N-D
    /// array is refused for having pages rather than for the shape of any one of them.
    /// </summary>
    internal const string MPowerDimensionRefusal =
        "Arguments must be 2-D. To operate on each element of the matrix individually, use POWER "
        + "(.^) for elementwise power.";
    ...
    /// <summary>
    /// <c>s ^ A</c>: a scalar raised to a square matrix, which is the matrix function
    /// <c>f(x) = s^x</c> and not the elementwise power this used to answer (M142).
    /// </summary>
    /// <remarks>
    /// <para>
    /// The mirror case, <c>A ^ p</c>, has been the repeated product since the beginning; this is
    /// the other half of <c>mpower</c>, and until M142 it fell through to <c>.^</c> and answered
    ...
    internal static JgsValue ScalarMatrixPower(JgsValue scalar, JgsValue matrix, int line, int col)
    {
        ...
        {
            throw new JgsRuntimeException(line, col, MPowerDimensionRefusal);
        }
        ...
        try
        {
            return FromComplexRect(MatrixFunction.ScalarPower(
                baseValue, ComplexSquareOf("'^'", matrix, line, col), MatlabPower));
        }
