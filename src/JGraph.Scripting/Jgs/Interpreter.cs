...
internal sealed partial class Interpreter
{
    ...
    private JgsValue ApplyBinary(TokenType op, JgsValue left, JgsValue right, Node at)
    {
        ...
        // problem and name the wrong fix (M139).
                                     ...
                                     JgsNumericClass into, out bool alreadyInClass)
    {
        ...
        if (Dialect.IsMatlab)
        {
            ...
            {
                // `s ^ A` is the other half of mpower, and the half that had never been written:
                // a scalar raised to a matrix is MATLAB's eigendecomposition
                // `V * diag(s .^ diag(D)) / V`, not the elementwise power this used to fall
                // through to (M139). ScalarForm says the left side is the scalar one for '^',
                // which is exactly the shape that reaches here.
    ...
    private JgsValue MatrixOperation(TokenType op, JgsValue left, JgsValue right, Node at)
    {
        if (op == TokenType.Caret)
        {
            ...
            // here: the two shapes that '^' does read answer above, in MATLAB's own words (M139).
    ...
    /// <summary>
    /// Whether an operand is the matrix half of MATLAB's <c>s ^ A</c> — an array of something other
    /// than the one element that would make it a scalar (M139).
    /// </summary>
    /// <remarks>
