...
public static class MatrixFunction
{
    ...
    /// <summary>
    /// MATLAB's <c>s ^ A</c> for a scalar <paramref name="scalar"/> and a real square matrix:
    /// <c>[V, D] = eig(A); V * diag(s .^ diag(D)) / V</c>, written out exactly as MATLAB writes it.
    /// </summary>
    ...
    /// diagonal here is spelt the same way <c>.^</c> is spelt everywhere else.
    /// </param>
    /// <remarks>
    /// <para>
    /// This is a different function from <see cref="Evaluate"/>, and deliberately: MATLAB does not
    /// reach for Schur-Parlett here, and it does not reach for <c>expm(log(s)*A)</c> either. The
    /// two disagree — on the defective <c>[2 1; 0 2]</c> the exponential form is off by nine — and
    /// the eigendecomposition is the one that reproduces MATLAB bit for bit on the well-conditioned
    /// cases. Transcribing the formula is therefore the whole of the algorithm, roundoff included.
    /// </para>
    /// <para>
    /// A defective matrix has a near-singular eigenvector matrix and the answer is then dominated
    /// by whatever the eigensolver put in the columns it could not separate. MATLAB has the same
    /// problem and gives no warning about it; the answers agree only as far as the two eigensolvers
    /// agree, which on a repeated eigenvalue is not far. That is a property of the formula and not
    /// of this transcription of it.
    /// </para>
    /// <para>
    /// The real branch is not an optimization. A real problem — a real base, real eigenvalues, real
    /// powers — is one MATLAB carries out in real arithmetic, and the difference shows: complex
    /// arithmetic turns <c>0 · Inf</c> into a NaN imaginary part where the real product has none,
    /// so <c>0 ^ [1 2; 3 4]</c> would come back complex instead of the real matrix of infinities
    /// MATLAB answers with.
    /// </para>
    /// </remarks>
    public static Complex[,] ScalarPower(Complex scalar, double[,] matrix, Func<Complex, Complex, Complex> power)
    {
        ...
        Eigen eigen = Eigen.Factor(matrix);
        Complex[] diagonal = RaisedDiagonal(scalar, eigen.Values, power);

        if (scalar.Imaginary == 0 && AllReal(eigen.Values) && AllReal(diagonal))
        {
            ...
            for (int c = 0; c < n; c++)
            {
                for (int r = 0; r < n; r++)
                {
                    vectors[r, c] = eigen.Vectors[r, c].Real;
                }
            }

            var scaling = new double[n, n];
            for (int i = 0; i < n; i++)
            {
                scaling[i, i] = diagonal[i].Real;
            }

            // V * diag(p) is written as the full product it is in MATLAB rather than as a column
            // scaling: the zeros off the diagonal are multiplied too, and 0 * Inf is the NaN that
            // makes `2 ^ [Inf 0; 0 1]` all NaN there and would not appear in a scaling.
            double[,] scaled = Linear.Multiply(vectors, scaling);

            // X / V is (Vᵀ \ Xᵀ)ᵀ, the same shape the '/' operator itself takes.
            double[,] solved = Linear.Solve(Linear.Transpose(vectors), Linear.Transpose(scaled));
            double[,] real = Linear.Transpose(solved);

            var widened = new Complex[n, n];
            for (int c = 0; c < n; c++)
            {
                for (int r = 0; r < n; r++)
                {
                    widened[r, c] = new Complex(real[r, c], 0);
                }
            }
        ...
        return Recombine(eigen.Vectors, diagonal);
    }

    /// <summary>The same, for a matrix that is genuinely complex.</summary>
    public static Complex[,] ScalarPower(Complex scalar, Complex[,] matrix, Func<Complex, Complex, Complex> power)
    {
        ArgumentNullException.ThrowIfNull(matrix);
        ArgumentNullException.ThrowIfNull(power);
        int n = matrix.GetLength(0);
        ...
        (Complex[] values, Complex[,] vectors) = ComplexEigen.Factor(matrix);
        return Recombine(vectors, RaisedDiagonal(scalar, values, power));
    }

    /// <summary><c>s .^ diag(D)</c> — the diagonal the eigenvectors are wrapped around.</summary>
    private static Complex[] RaisedDiagonal(Complex scalar, Complex[] values, Func<Complex, Complex, Complex> power)
    {
        var raised = new Complex[values.Length];
        for (int i = 0; i < values.Length; i++)
        {
            raised[i] = power(scalar, values[i]);
        }

        return raised;
    }

    /// <summary><c>V * diag(p) / V</c> in complex arithmetic.</summary>
    private static Complex[,] Recombine(Complex[,] vectors, Complex[] diagonal)
    {
        int n = diagonal.Length;
        var scaling = new Complex[n, n];
        for (int i = 0; i < n; i++)
        {
            scaling[i, i] = diagonal[i];
        }

        Complex[,] scaled = ComplexLinear.Multiply(vectors, scaling);
        if (!ComplexLinear.TrySolve(TransposePlain(vectors), TransposePlain(scaled), out Complex[,] solved))
        {
            throw new InvalidOperationException("The matrix is singular to working precision.");
        }

        return TransposePlain(solved);
    }
    ...
    private static bool AllReal(Complex[] values)
    {
        foreach (Complex value in values)
        {
