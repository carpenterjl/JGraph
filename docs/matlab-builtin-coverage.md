...
Across every callable kind — builtin, function, operator, keyword, script — the count is **1,123 of
2,024** as of V6's table-forms sub-stage (ADR 0167), which added `table2array`, `varfun`, `rowfun` and `istable`, beside `addvars`, which the list does not name; 1,119 as of V6's twelfth sub-stage, which added `events`, the list of a class's events, beside `notify`, `addlistener` and `listener`, which the list does not name; 1,118 as of V6's tenth sub-stage, which added `guidata` and `isvalid`, the two verbs the value-ownership probes found missing beside `axes(parent)`; 1,117 as of ADR 0150, which added `hist`, the legacy histogram a folder of user scripts still called; 1,116 as of M145, which added `builtin` — the forwarder that reaches a built-in past the file
...
wave and one milestone after it: the
five parallel milestones M127–M131 added the quadrature above one dimension, the sparse iterative
solvers and orderings, the scattered-data interpolants, the boundary-value and delay solvers, and
`pdepe`; M132 added the Signal Processing Toolbox's windows and generators, which are counted by
`matlab-signal-coverage.md` rather than here, and three base names beside them (`chirp`, `db2mag`,
`mag2db`). The count stood at **1,054** as of M126, which added the stiff ODE family (`ode15s`,
`ode23s`, `ode23t`, `ode23tb`) and the fully implicit pair (`ode15i`, `decic`) — six names, and the
first of them that can solve a differential-algebraic equation rather than only an ordinary one.
M125 added five before them (`ode23`, `ode78`, `ode89`, `ode113`, `odextend`), and the commits
between the two milestones added four more without a count of their own.

The count was **1,039** as of M123, which added `deval` — the one name that lets a solution
structure be read at a time nobody asked for while it was being computed. The 1,038 before it came from M122's capability
...
five of MATLAB's seven profiles are written (the two JPEG 2000 ones are refused by name). M112 added
two more after MATLAB's seven, `'Animated PNG'` and `'Uncompressed AVI with Alpha'`, because not one
of MATLAB's carries an alpha channel and a script that has drawn a cut-out otherwise has nowhere to
put it (ADR 0114). `open`
...
name.
947 after M100, which took the polynomial and 1-D signal names: `roots` `poly` `polyder`
