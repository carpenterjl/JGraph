...
Across every callable kind — builtin, function, operator, keyword, script — the count is **1,118 of
2,024** as of V6's tenth sub-stage (ADR 0167), which added `guidata` and `isvalid`, the two verbs the value-ownership probes found missing beside `axes(parent)`; 1,117 as of ADR 0150, which added `hist`, the legacy histogram a folder of user scripts still called; 1,116 as of M145, which added `builtin` — the forwarder that reaches a built-in past the file
