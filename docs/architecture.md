...
| `JGraph.Numerics` | net8.0 | Flat contiguous numeric storage for large datasets: the dual-strategy `NumericBuffer` (managed / `NativeMemory` / memory-mapped temp file, chosen by the RAM-aware `BufferAllocator`) and the chunked, cancellable `PackedMath` SIMD kernels over `TensorPrimitives`. Each unary kernel carries a determinism tier (ADR 0092): `Exact` where the vector form and the scalar loop agree bit for bit and is wired unconditionally, `Approximate` where they differ by a few ulps and waits behind `ApproximateThreshold` (32K elements, `JGRAPH_FAST_MATH=0` to disable — ADR 0093), with `UnaryTiered` making the choice so callers need not know which is which. Those kernels sweep their buffers through `ParallelKernels.For` in fixed 64K grains — a decomposition that depends on length alone, so a thread count is never an input to an answer — on up to `MaxDegree` threads (`JGRAPH_THREADS`, default logical processors capped at 16, which is the opposite of the native side's physical-core rule and for the reason ADR 0093 records); the cancellation poll runs between grains and `OperationCanceledException` comes back out unwrapped, while the reductions that fold values stay serial and in index order. The dimension reductions carry their own kernels in `ReduceKernels` (ADR 0094): a reduction along one dimension decomposes as `(inner, n, outer)`, every output is folded whole by one thread in the boxed fold's own order — contiguous slices when the dimension is the first, one running accumulator per output across each interleaved panel otherwise — over blocks `ParallelKernels.ForBlocks` cuts at boundaries that are a function of the shape alone, so `sum(A, 1)`, `max(A, [], 2)`, `cumsum` and their kin answer the boxed bits at any thread count. `sort` has its own kernels in `SortKernels` (ADR 0095) over the same decomposition: one long slice is cut into buckets by value — splitters from a strided sample, a counting pass, a stable scatter, then every bucket sorted on its own thread with nothing to merge, because the buckets already sit in order end to end — while many short slices take a thread each. What a sort has to defend is not a fold order but a tie rule: values compared with `<` so the two zeros tie, equal values left in the order they arrived whichever direction is asked for, NaN lifted out and put back where `MissingPlacement` names it with its own bits intact. The discrete Fourier transform is `FftKernels` (ADR 0096), over planar storage — real parts in one span, imaginary in another — so a butterfly is doubles rather than boxed `Complex` and eight signals ride one SIMD register side by side; the bit-reversal, stage order, twiddle table and product spelling are the pre-M96 radix-2's, so every length under 32K points answers the old bits, while a longer one is factored into two batched passes with the transpose folded into the gather's stride and rounds differently by length alone. `FilterKernels` does `filter(b, a, x)` where the denominator has nothing past `a(1)`: the recurrence unrolls to one right-nested chain per output, so vectorising goes across outputs and never across taps and the bits are the recurrence's own — except that a NaN now reaches only as far as the filter is long, which is what MATLAB does and what `0 · y` did not. A numeric class is a rule the kernels carry rather than a sweep of their own (ADR 0098): `PackedMath.Rounding` is `single`'s round to float precision, or an integer class's round half away from zero, saturate into the class range and read NaN as zero — spelled once as a scalar and once in vector registers, comparing the fraction against a half rather than adding one to the element and selecting the step away from zero rather than adding it, so that neither a value one ulp under a tie nor a negative signed zero moves where `Math.Round` would leave it. The elementwise kernels take that rule as an argument and apply it to each tile in the pass that computed the tile, which is what makes an `int32` addition cost what a `double` addition costs. Dense linear algebra answers through the `DenseLinalg` provider seam (ADR 0088–0091): the bundled OpenBLAS (`native\win-x64`, loaded and configured by `OpenBlasLoader`, one thread per physical core) when it loads, the managed kernels otherwise, forced either way by `JGRAPH_LINALG`. The seam carries the product and the symmetric rank-k product, the LU family with its solves, inverse and condition estimate, Cholesky, the triangular solve, least squares, Householder QR with and without pivoting and the two ways of applying its reflectors, the singular value decomposition with its fallback driver, the symmetric and general eigensolvers, the generalized (pencil) eigensolvers, the real Schur form with its reorder, the QZ factorization, and the complex z-family (product, LU trio, eigensolver, SVD — `Span<Complex>` crosses the boundary as LAPACK's own interleaved layout); `LuDecomposition`, `Cholesky`, `QrDecomposition`, `Svd` and `Eigen` hold their factors flat and column-major, which is both LAPACK's layout and the script's. Since M99 (ADR 0100) `Optimization/` holds the derivative-free solvers base MATLAB's `optimfun` folder ships: Nelder-Mead (`SimplexSearch`), Brent's `fmin` (`BoundedMinimizer`) and `zeroin` (`RootFinder`), and Lawson-Hanson non-negative least squares, all over a shared `SearchWatcher` seam a display or a script's own output function can watch a solve through and stop it. `SimplexSearch` is deliberately a second Nelder-Mead beside `JGraph.Statistics.Optimize.NelderMead`, which serves the distribution fitters and differs from MATLAB in six ways a fitter cannot see and a script can. M100 (ADR 0101) adds the polynomial and 1-D signal arithmetic: `Polynomials` (roots via the companion matrix's eigenvalues, so the question is `eig`'s and the accuracy is `eig`'s; expansion from roots, derivative and antiderivative, the matrix polynomial, and long division), `Convolution` (direct and never transformed, because an FFT route leaves dust where the direct sum leaves exact zeros, over sequences and over arrays of any rank with MATLAB's three shapes), `PhaseSequences` (unwrapping a phase record, each step measured against the record as it arrived; and ordering complex values into conjugate pairs) and `PlanarGeometry` (polygon area, rectangle overlaps, and a point-in-polygon test that counts winding by quadrant rather than by angle, so no arctangent decides a point sitting on an edge). M101 (ADR 0102) adds the interpolation half beside it: `Interpolation` gains the modified Akima slopes, the clamped spline, Keys' cubic-convolution weights with the quadratic edge sample they need, and the conversion from slopes to the four coefficients per piece that MATLAB's `pp` structure holds; `GridSampling` reads a plaid grid at a point in any number of directions, the three local methods as a product of per-direction stencil weights and the spline as a tensor Hermite over slopes taken once along each combination of directions — which is well defined because a not-a-knot spline's slopes are linear in the samples and is exactly why `pchip` and `makima` cannot be done the same way; and `FourierResampling` reads a record at n evenly spaced places over its own period, splitting an even-length record's Nyquist bin in half so that a real record stays real. M102 (ADR 0103) adds the named matrices themselves: `TestMatrices` holds the classical ten — Toeplitz, Hankel, Vandermonde, companion, Pascal's three forms, Hadamard, Wilkinson, the inverse Hilbert and Rosser — and `GalleryMatrices` the forty-two Higham families that are decided by their arguments alone. Every one of them is written as a rule about its own two indices straight into column-major storage, so nothing is assembled from smaller pieces and nothing is transposed; the inverse Hilbert is carried by its recurrence rather than its closed form, whose three binomial coefficients each overflow a double long before their quotient does. The families whose angles are rational parts of a turn take them in degrees through `CosineOfDegrees`, which reduces with a two-part `π/180` and carries the product's own rounding error: that makes the quarter turns exact, so a root of unity is `i` and not `6.1 × 10⁻¹⁷ + i`, and it cuts the disagreement with the correctly rounded cosine over a Chebyshev grid from 55 % of its points to 17 %. M103 (ADR 0104) adds `ChangePoints`: where a signal stops being one thing, asked three ways — did the mean move, the spread, or the slope — and answered once, by the exact dynamic programme over prefix sums with a penalty per cut; a penalty-driven search keeps the earliest of two equally-priced splits, a change budget keeps the latest and spends only what improves the residual, M106 (ADR 0107) closes MATLAB's `specfun` folder with five more: `EllipticFunctions` (the complete integrals and the Jacobi functions, both by the arithmetic-geometric mean), `ExponentialIntegral`, `LegendreFunctions`, `ContinuedFractions` and `Assignment` (minimum-cost perfect matching by shortest augmenting paths). The first three take the whole array rather than one element at a time, and that is a correctness rule and not an optimisation: each stops when the largest remaining correction anywhere in the array is under the tolerance, so a settled element goes on receiving corrections that are small but not zero, and reading an array one element at a time would answer different last bits. `LegendreFunctions` computes fully normalized whatever normalization was asked for and scales at the end — as a running product through each element when the factor alone would overflow, which is the only reason degree 150 comes back finite in all 151 rows. M107 (ADR 0108) closes MATLAB's `matfun` folder with the layer above the factorizations rather than more of them: `PlaneRotations` (the Givens rotation and the column and row updates of a QR factorization built from it), `RowEchelon`, `SchurConversion` (between a real quasi-triangular form and a complex triangular one, both ways), `SylvesterEquation` (Bartels-Stewart back-substitution over blocks of at most two a side), `NormEstimators` (a power iteration for the two-norm and Higham and Tisseur's block algorithm for the one-norm of an operator that can only be applied), `HouseholderQr` (a complex pivoted factorization and the complete orthogonal decomposition over it), `MatrixFunction` (Schur-Parlett, blocking eigenvalues by closeness so the recurrence never divides by a small difference) and `GeneralizedSvd` (Van Loan's CS decomposition). Two things there are worth knowing about: `PlaneRotations.Length` computes a two-vector's length correctly rounded, by a Newton step over a residual every term of which is recovered exactly, because `double.Hypot` disagrees with MATLAB's `norm` in an eighth of measured pairs and `planerot`'s radius is documented to be that `norm`; and `ComplexEigen.Schur` accumulates the unitary the eigenvalue iteration was already applying, which is what a complex `schur` needed and which a real matrix does not use — the real form plus `rsf2csf` is cheaper and is what MATLAB does. The only project that compiles unsafe code or touches native math. A leaf; consumed by `JGraph.Scripting`. |
...
| `JGraph.Imaging.Codecs` | net8.0 | Image file decoding/encoding (PNG/JPEG/BMP) via SkiaSharp, bridging bytes to `ImageBuffer`, and the video muxers behind `VideoWriter` (AVI, MPEG-4 via Media Foundation, animated PNG). Both halves of the APNG work live here: `AnimatedPngEncoder` writes one and `AnimatedPngReader` plays one back, because Skia decodes neither — handed an APNG its codec reports a single still, which is what the format asks an unaware decoder to do (ADR 0114, ADR 0115). The only image project that touches a native codec; referenced directly by `JGraph.Scripting` and `JGraph.Application`. |
...
| `JGraph.Scripting` | net8.0 | Scripting hosts: the `IScriptEngine` seam, a Roslyn C# engine, a pythonnet (CPython) engine, and the built-in **JGS** language (a self-contained lexer/parser/interpreter under `Jgs/`). The same pipeline runs **MATLAB** (`.m`) as a second dialect: a `JgsDialect` record threads through the lexer, parser, and interpreter, and `MatlabScriptEngine` fixes it to MATLAB (1-based, `%` comments, `function` defs, cells/structs, value semantics) — see ADR 0031. A bracket holding a string array is a string array however it is written: the single-row join has been there since M63 and the multi-row block join since M104 (ADR 0105), so `["a"; "b"]` is a 2-by-1 string array and not the 2-by-1 double it used to be. There is one alignment engine for two operands and only one: `JgsBroadcast` serves the elementwise operators, `bsxfun`, and — since M106 (ADR 0107) — the two-argument numeric builtins as well, which until then compared lengths rather than shapes and answered packed storage carrying no shape at all, so `atan2` of a 2-by-3 came back a row and `atan2` of a column and a row was refused outright. A MATLAB `for` over a range or a `while` whose body works entirely in scalar doubles compiles once (`LoopCompiler` → `RegisterProgram`, ADR 0099) to a linear op array over an unboxed double register file and runs without the tree walk — same arithmetic bound from the same scalar kernels, step counting and cancellation preserved, variables spilled to the environment exactly where the walk would have left them, and every case a register cannot hold (an answer leaving the reals, an error the walk would throw, a shadowed builtin) handed back to the walk; `JGRAPH_LOOP_JIT=0` disables it, and the two roads must print byte-identical output. `decomposition` (M107, ADR 0108) is a struct wearing a class name, the way every object in this build is carried, with its factors held in `DecompositionFactors` beside it; the operators it answers to — backslash, forward slash, scalar multiplication and division, negation and the conjugate transpose — are claimed before the struct is read as anything else, by one hook each in `ApplyBinaryCore`, `EvaluateUnary` and `EvaluateTranspose`. Since M145 (ADR 0149) every name in the MATLAB dialect is decided by one `JgsNameResolver` in MATLAB's own order — bound names, nested and local functions, the built-in's class methods (a 2,159-name R2025b dispatch table, `JgsDispatchTable`), `private/`, user methods, the current folder, the path, and last the built-ins, which live in a sealed `JgsBuiltinLayer` under the base workspace; function storage lives with its `FunctionFile`, a `NamedHandle` captures the layer that answered, `JgsFileIndex` answers whether a folder shadows a built-in without touching the disk, and `builtin(name, …)` is a transparent forwarder over the layer. The `JGraphScriptGlobals` bridge drives the `JG` API and the `Table` readers. Also the UI-free `Workspace/` layer: `ScriptWorkspace` (folder enumeration, watcher, bare-filename resolution) and the session/document state models behind the scripting window. WPF-free, no dependency for JGS. Also `Startup/`: the shared command-line parser, statement resolution, output sinks (console/file/tee) and `BatchRunner` that both executables run. |
