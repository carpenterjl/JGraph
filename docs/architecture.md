...
| `JGraph.Numerics` | net8.0 | Flat contiguous numeric storage for large datasets: the dual-strategy `NumericBuffer` (managed / `NativeMemory` / memory-mapped temp file, chosen by the RAM-aware `BufferAllocator`) and the chunked, cancellable `PackedMath` SIMD kernels over `TensorPrimitives`. Dense linear algebra answers through the `DenseLinalg` provider seam (ADR 0088): the bundled OpenBLAS (`native\win-x64`, loaded and configured by `OpenBlasLoader`) when it loads, the managed kernels otherwise, forced either way by `JGRAPH_LINALG`. The only project that compiles unsafe code or touches native math. A leaf; consumed by `JGraph.Scripting`. |
