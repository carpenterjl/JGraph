...
Closed since, the same way: **`s ^ A` answered elementwise** — `2 ^ [1 2; 3 4]` was `[2 4; 8 16]`
here against R2025b's `[10.48 14.15; 21.23 31.71]`. ADR 0146 settled it, and by a matrix function
rather than by the eigendecomposition this bullet named: the eigendecomposition is what MATLAB does
and it is wrong on a defective exponent, so what is left of the entry is a divergence recorded
there, where `2 ^ [2 1; 0 2]` is `[4 4·ln2; 0 4]` here and `[4 0; 0 4]` in MATLAB.
