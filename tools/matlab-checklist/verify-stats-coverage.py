#!/usr/bin/env python3
"""Check that docs/matlab-stats-coverage.md agrees with the R2021b list and with the live catalog.

The sibling of verify-ipt-coverage.py, for the Statistics and Machine Learning Toolbox. The base
tracker (build-checklist.py) reads a scraped install dump that holds no toolbox rows at all, so the
stats list is kept in matlab-r2021b-stats.csv and the coverage doc is written by hand against it.
A hand-maintained doc over a separate list is exactly where counts rot, so this script is the guard:
...
CSV_PATH = REPO / "tools/matlab-checklist/matlab-r2021b-stats.csv"
DOC_PATH = REPO / "docs/matlab-stats-coverage.md"
