...
work = fullfile(root, 'dispatch_work5');
...
out = builtin('fopen', fullfile(root, 'dispatch-probe5.csv'), 'w');
builtin('fprintf', out, 'name');
for p = 1:builtin('size', patterns, 1), builtin('fprintf', out, ',"%s"', patterns{p, 1}); end
builtin('fprintf', out, '\n');
sentinel = -12345;
for k = 1:numel(rows)
    name = rows{k};
    ...
    for p = 1:builtin('size', patterns, 1)
        args = patterns{p, 2};
        verdict = 'builtin';
        try
