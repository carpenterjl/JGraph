...
public sealed class JgsCompletionTests
{
    ...
    public void Catalog_RendersSignatures_WithOptionalMarkers()
    {
        ...
        Assert.NotNull(plot);
        // M77 gave plot the option tail every other chart verb has had since M42, so the trailing
        // name-value pairs show in the signature the way figure's have.
        Assert.Equal("plot(x, y, spec?, options?)", plot!.Signature);
    ...
    public void SignatureHelp_ShowsTheCall_WithTheFirstParameterActive()
    {
        ...
        Assert.Equal(new[] { "x", "y", "spec?", "options?" }, help.ParameterLabels);
